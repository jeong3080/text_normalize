...
import json
import time
with open('tn_dic_v5.json', 'r') as f:
    tn_dic = json.load(f)
...
count_checker = tn_dic["unit"]["unit_list"]#"(시|명|가지|살|마리|포기|송이|수|톨|통|점|개|벌|척|채|다발|그루|자루|줄|켤레|그릇|잔|마디|상자|사람|곡|병|판|번째) 등"
count_checker_eng_1 = tn_dic["unit"]["unit_list_eng_1"]
count_checker_eng_2 = tn_dic["unit"]["unit_list_eng_2"]
...
def normalize(text):
    ...
    if "." in text or "-" in text or "@" in text:
        text = normalize_special_format(text)
...
def unit_english_to_korean_1(text):
        num_str, unit_str = text.group(1), text.group(2)
        if unit_str == "":
            return text
        else:
            return num_str+tn_dic["unit_to_kor1"][unit_str]

def unit_english_to_korean_2(text):
        num_str, unit_str = text.group(1), text.group(2)
        if unit_str == "":
            return text
        else:
            return num_str+tn_dic["unit_to_kor2"][unit_str]
...
def normalize_special_format(text):
    text = re.sub(email_checker, lambda x: normalize_email_format(x),text)
    text = re.sub(phone_checker, lambda x: normalize_phone_format(x),text)
    text = re.sub(ip_checker, lambda x: normalize_ip_format(x),text)
    return text
...
def normalize_ip_format(text):
    result = "".join([ip_to_kor[i] for i in text.group()])        
    return result