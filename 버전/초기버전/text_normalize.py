...
def normalize(text):
    ...
    text = re.sub("[a-zA-Z]+", normalize_univerary_english, text)
...
def normalize_univerary_english(text):
    text = text.group(0)
    text = text.lower()
    return CMUToKorean.convert(text)[0]
...
count_checker = "(시|명|가지|살|마리|포기|송이|수|톨|통|점|개|벌|척|채|다발|그루|자루|줄|켤레|그릇|잔|마디|상자|사람|곡|병|판|번째)"
